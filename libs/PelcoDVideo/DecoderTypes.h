...
/// @brief Interface for in-place post-processing of decoded video frame buffers.
class IFrameProcessor {
public:
    virtual ~IFrameProcessor() = default;

    /// @brief Processes a decoded frame's raw pixel buffer in-place.
    /// @param[in,out] data Pointer to raw frame buffer (packed RGB24/BGR24 format).
    /// @param[in] width Frame width in pixels.
    /// @param[in] height Frame height in pixels.
    /// @param[in] format Current pixel layout of the frame buffer.
    virtual void process(std::uint8_t* data, int width, int height, PixelFormat format) = 0;
};
...
[[nodiscard]] inline SourceType detectSourceType(std::string_view source) noexcept
{
    ...
    if (source.rfind("rtsp://", 0) == 0 || source.rfind("rtmp://", 0) == 0 || source.rfind("http://", 0) == 0
        || source.rfind("https://", 0) == 0 || source.rfind("udp://", 0) == 0 || source.rfind("tcp://", 0) == 0) {
        return SourceType::Rtsp;
    }
    if (source.rfind("video=", 0) == 0 || source.rfind("video:", 0) == 0 || source.rfind("device:", 0) == 0
        || source.rfind("device://", 0) == 0 || source.rfind("dshow:", 0) == 0 || source.rfind("/dev/video", 0) == 0) {
