...
public:
    ...
    bool setRate(double panDegPerSec, double tiltDegPerSec) override {
        return setNormalizedVelocity(static_cast<float>(panDegPerSec / 60.0),
                                     static_cast<float>(tiltDegPerSec / 30.0));
    }

    bool setNormalizedVelocity(float panVel, float tiltVel) override {
        std::lock_guard<std::mutex> lock(m_mutex);
        m_telemetry.panRateDegPerSec = std::clamp(panVel, -1.0f, 1.0f) * 60.0;
        m_telemetry.tiltRateDegPerSec = std::clamp(tiltVel, -1.0f, 1.0f) * 30.0;
    ...
    bool supportsStabilization() const noexcept override { return true; }
    ...
    bool zeroGyroDrift() override { return true; }
...
public:
    explicit SimCamera(CameraSpectrum spec) : m_spectrum(spec) {
        updateFov();
    }
    ...
    CameraSpectrum spectrum() const noexcept override { return m_spectrum; }
    ...
    bool zoomStop() override { return true; }
    ...
    bool triggerOnePushFocus() override { return true; }
...
public:
    ...
    bool connect() override { m_connected = true; return true; }
    void disconnect() override { m_connected = false; disarmLaser(); }
    bool isConnected() const noexcept override { return m_connected; }
    DeviceState state() const noexcept override {
        return m_connected ? DeviceState::Ready : DeviceState::Disconnected;
    }
