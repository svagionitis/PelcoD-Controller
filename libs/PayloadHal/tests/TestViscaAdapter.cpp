...
namespace {
    ...
    TEST(TestViscaAdapter, LifecycleAndOpticsMapping)
    {
        ...
        // Synchronous telemetry query
        const CameraTelemetry telem = adapter.currentTelemetry();
        EXPECT_GE(telem.opticalZoomFactor, 1.0);
        EXPECT_TRUE(telem.dayNightIcrActive);
