...
- **Ten Modular Subsystems:**
  ...
  2. [`libs/PelcoDCore/`](libs/PelcoDCore/): Pure C++17 protocol engine for standard Pelco-D (**zero vendor extensions, math, or socket dependencies**). Encapsulates framing, checksums, accumulator stream parser, abstract `ITransport` port, kinematics and latency simulation, `MockPelcoDDevice`, `BusScanner` with multi-baud discovery, `PatrolController`, `RttProfiler`, `ScopedConnectionList`, async queries, and packet macro scripting.
