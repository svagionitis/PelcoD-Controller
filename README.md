...
- **Eleven Modular Subsystems:**
  ...
  2. [`libs/PelcoDCore/`](libs/PelcoDCore/): Pure C++17 protocol engine for standard Pelco-D (**zero vendor extensions, math, simulation, or socket dependencies**). Encapsulates framing, checksums, accumulator stream parser, abstract `ITransport` port, `BusScanner` with multi-baud discovery, `PatrolController`, `RttProfiler`, `ScopedConnectionList`, async queries, and packet macro scripting.
