...
- **Eleven Modular Subsystems:**
  ...
  2. [`libs/PelcoDCore/`](libs/PelcoDCore/): Pure C++17 protocol engine for standard Pelco-D (**zero vendor extensions, math, simulation, or socket dependencies**). Encapsulates framing, checksums, decoupled byte stream accumulation (`RxStreamAccumulator`), prioritized command pacing with exponential retry backoff (`PacedCommandQueue`), abstract `ITransport` port, `BusScanner` with multi-baud discovery, `PatrolController`, `RttProfiler`, `ScopedConnectionList`, async queries, and packet macro scripting.
  ...
  4. [`libs/PelcoDFujinon/`](libs/PelcoDFujinon/): Specialized camera protocol extensions for Fujinon SX800 and SX801 series. Subclasses `PelcoDDevice` with proprietary packet encoders/decoders, optical defog, DayNightEx, Optical Image Stabilization (OIS), focal length telemetry, and real-time clock synchronization.
  5. [`libs/PelcoDSim/`](libs/PelcoDSim/): Virtual device emulation and kinematics simulation library. Encapsulates in-memory virtual hardware emulator (`MockPelcoDDevice`), physical motor kinematics simulation (`KinematicsSimulator` with acceleration/slew modeling), and link latency/jitter/packet loss pipeline (`LatencyPipeline`).
  6. [`libs/Tracking/`](libs/Tracking/): Dedicated closed-loop tracking, kinematics estimation, and dynamics control library. Encapsulates 3-axis PTZ auto-tracking (`PtzAutoTracker`), non-linear state estimation via Extended Kalman Filtering (`ExtendedKalmanFilter`) and Unscented Kalman Filtering (`UnscentedKalmanFilter`), 3D spherical kinematic state estimation (`PtzSphericalEstimator`), projective pinhole camera geometry with optical zoom and radial distortion (`PtzCameraModel`), 1D/3-axis PID control with anti-windup and derivative filtering (`PidController`), real-time command-to-video latency cross-correlation estimation (`LatencyEstimator`, `LatencyCalibrator`), servo hunting oscillation detection (`OscillationDetector`), plant transfer function identification (`PlantIdentifier`, `ChirpCalibrator`), and transient mechanical shock detection (`TransientShockDetector`).
  7. [`libs/Video/`](libs/Video/): Lightweight multi-backend video streaming pipeline supporting RTSP, local video files, and camera capture devices (**zero OpenCV dependencies**). Features atomic triple buffering (`AtomicTripleBuffer`), modular decoders (FFmpeg, GStreamer, Mock SMPTE), camera enumeration (`DeviceEnumerator`), pure frame processing port (`IFrameProcessor`), and terminal Braille rendering (`BrailleRenderer`).
  8. [`libs/VideoFilters/`](libs/VideoFilters/): Standalone OpenCV-based computer vision and image enhancement library implementing `IFrameProcessor`. Modularized into 6 domain-specific categories: `ColorFilters` (CLAHE, HistEq, AWB, tone curves), `SpatialFilters` (unsharp masking, atmospheric DCP dehazing, temporal scintillation denoise, lens distortion and chromatic aberration corrections), `GeometricFilters` (mirror, mosaic pixelation, local area processing, electronic image stabilization, picture-in-picture), `ThermalFilters` (false-color thermal palettes, isotherm temperature bands, hotspot radiometry), `TrackingFilters` (moving target indicator MTI, optical flow velocity fields, centroid Kalman target tracking with predictive lead, perimeter tripwire, motion heatmaps), and `OverlayFilters` (tactical HUD reticles, military mil-dot scales, privacy masking, evidential timestamp watermarks, telemetry OSD).
  9. [`libs/Onvif/`](libs/Onvif/): Standalone ONVIF Profile S and Profile T client and server library (zero Qt dependencies). Implements WS-Discovery multicast scanning, WS-Security password digest generation, Device Management, Media Streaming (RTSP & Snapshot URIs), PTZ controls (continuous, relative, absolute, home position, presets, tours, GeoMove), Optical & Imaging Service (brightness, contrast, saturation, sharpness, IR cut filter, WDR, BLC, focus), thermal radiometry, and PullPoint Event Service (real-time motion/tamper notifications). Contains protocol hardware adapters in `libs/Onvif/adapters/`.
...
|---|---|---|
| `PELCOD_ENABLE_VIDEO` | `ON` | Compiles `Video` library and video decoding / tactical filter pipelines |
| `PELCOD_ENABLE_ONVIF` | `ON` | Compiles `Onvif` library, Profile S/T client, and ONVIF tabs/CLI tools |
...
## Standalone ONVIF Profile S & Profile T Subsystem (`Onvif`)

The [`libs/Onvif/`](libs/Onvif/) library is a pure C++17 client (zero Qt dependencies) conforming to ONVIF Profile S and Profile T specifications:
