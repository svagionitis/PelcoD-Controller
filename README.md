...
- **Nine Modular Subsystems:**
  ...
  2. [`libs/PelcoDCore/`](libs/PelcoDCore/): Pure C++17 protocol engine with **zero external math or socket dependencies**. Encapsulates framing, checksums, accumulator stream parser, abstract `ITransport` port, kinematics and latency simulation, `MockPelcoDDevice`, `BusScanner` with multi-baud discovery, `PatrolController`, `RttProfiler`, `ScopedConnectionList`, async queries, packet macro scripting, and specialized camera profiles (`FujinonSX800Device`).
