...
- **Eleven Modular Subsystems:**
  ...
  6. [`libs/PelcoDTracking/`](libs/PelcoDTracking/): Dedicated closed-loop tracking, kinematics estimation, and dynamics control library. Encapsulates 3-axis PTZ auto-tracking (`PtzAutoTracker`), non-linear state estimation via Extended Kalman Filtering (`ExtendedKalmanFilter`) and Unscented Kalman Filtering (`UnscentedKalmanFilter`), 3D spherical kinematic state estimation (`PtzSphericalEstimator`), projective pinhole camera geometry with optical zoom and radial distortion (`PtzCameraModel`), 1D/3-axis PID control with anti-windup and derivative filtering (`PidController`), real-time command-to-video latency cross-correlation estimation (`LatencyEstimator`, `LatencyCalibrator`), servo hunting oscillation detection (`OscillationDetector`), plant transfer function identification (`PlantIdentifier`, `ChirpCalibrator`), and transient mechanical shock detection (`TransientShockDetector`).
  7. [`libs/PelcoDVideo/`](libs/PelcoDVideo/): Lightweight multi-backend video streaming pipeline supporting RTSP, local video files, and camera capture devices (**zero OpenCV dependencies**). Features atomic triple buffering (`AtomicTripleBuffer`), modular decoders (FFmpeg, GStreamer, Mock SMPTE), camera enumeration (`DeviceEnumerator`), pure frame processing port (`IFrameProcessor`), and terminal Braille rendering (`BrailleRenderer`).
  8. [`libs/PelcoDVideoFilters/`](libs/PelcoDVideoFilters/): Standalone OpenCV-based computer vision and image enhancement library implementing `IFrameProcessor`. Modularized into 6 domain-specific categories: `ColorFilters` (CLAHE, HistEq, AWB, tone curves), `SpatialFilters` (unsharp masking, atmospheric DCP dehazing, temporal scintillation denoise, lens distortion and chromatic aberration corrections), `GeometricFilters` (mirror, mosaic pixelation, local area processing, electronic image stabilization, picture-in-picture), `ThermalFilters` (false-color thermal palettes, isotherm temperature bands, hotspot radiometry), `TrackingFilters` (moving target indicator MTI, optical flow velocity fields, centroid Kalman target tracking with predictive lead, perimeter tripwire, motion heatmaps), and `OverlayFilters` (tactical HUD reticles, military mil-dot scales, privacy masking, evidential timestamp watermarks, telemetry OSD).
